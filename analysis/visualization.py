...
def plot_in_by_out(input_folder):
    ...
    x, y, y2 = [], [], []
    samples = int(params['samples'])
    for dir in sorted(os.listdir(input_folder), key=lambda a: float(a)):
        print(dir)
        try:
            yl, yl2 = [], []
            for sample in range(samples):
                yl.append(calc_spectral(f'{input_folder}/{dir}/{sample + 1}/pearson', pot=1))
                yl2.append(calc_spectral(f'{input_folder}/{dir}/{sample + 1}/pearson', pot=2))
            x.append(float(dir))
            y.append(sum([i for i in yl]) / samples)
            y2.append(sum([i for i in yl2]) / samples)
        except Exception as e:
            print(e)
            continue

    plt.plot(x, y, label="pot 1")
    plt.plot(x, y2, label="pot 2")
    plt.text(1.1 * max(x), 0.6, information_message())
